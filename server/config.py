...
Online AI uses a fallback CHAIN across three OpenAI-compatible providers,
tried in this fixed order: OpenRouter -> Z.ai -> NVIDIA. Any provider
without an API key configured is skipped; if one provider errors out
(timeout, rate limit, etc.) the next one in the chain is tried
automatically before the whole request is considered failed.
"""
...
class Settings(BaseSettings):
    ...
    # --- Online AI fallback chain: OpenRouter -> Z.ai -> NVIDIA ---
    # llm_provider is kept for logging/reference of the "preferred" one;
    # the chain always tries all three configured providers regardless.
    ...
    def online_provider_chain(self) -> list[dict]:
        ...
        candidates = [
            {"name": "openrouter", "base_url": self.openrouter_base_url,
             "api_key": self.openrouter_api_key, "model": self.openrouter_model},
            {"name": "zai", "base_url": self.zai_base_url,
             "api_key": self.zai_api_key, "model": self.zai_model},
            {"name": "nvidia", "base_url": self.nvidia_base_url,
...
def validate_startup_config() -> list[str]:
    ...
    if not settings.online_provider_chain():
        warnings.append(
            "Tidak ada provider AI online yang dikonfigurasi (OPENROUTER_API_KEY / "
            "ZAI_API_KEY / NVIDIA_API_KEY semuanya kosong) — permintaan chat mode "
            "online akan gagal sampai minimal satu key diisi."
        )
